...
\newcommand{\w1}{\ensuremath{w_{user}}}
\newcommand{\w2}{\ensuremath{w_{similarUsers}}}
\newcommand{\w3}{\ensuremath{w_{restaurantAvg}}}
...
\[stars_{i} = \w1 * avg_{usr} + \w2 * similarUserAvg_{i} + \w3 * avg_{i} \]
 where $\w1 + \w2 + \w3 = 1$. $similarUserAvg_i$ was average review for restaurant $i$ from the similar users. Not all similar users would have reviewed the restaurant so this average may have been over as few as 1 to 2 similar users. 
\\[0.5em]
...
Our basic model relied on five input parameters: the $K_{KNN}$ for KNN, the number of principal components ($P$) and three weights applied to our three parameters used to make a prediction (\w1, \w2, \w3). The 
...
For our baseline, we used $\w1=.5$, $\w2=0$, and $\w3=.5$. Our goal was to see how our algorithm would perform without any knowledge of similar users, so we kept \w1 and \w3 weighted evenly, but removed \w2 entirely.

For the three weights, we chose to make the weights even: $\w1=.33$, $\w2=.33$, and $\w3=.33$. This meant that the similar users were factored in but we kept the user average and restaurant average to keep the predictions reasonably close to those averages.
